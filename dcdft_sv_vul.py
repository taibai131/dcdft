...
# Open data
t, dB = np.loadtxt("SV_Vul.dat", unpack=True)
avgdB = np.mean(dB)
...
plt.figure(0)
plt.plot(t, dB, 'k.')
plt.plot([np.min(t),np.max(t)],[avgdB,avgdB], 'b-', label="Mean $\Delta$B")
plt.legend()
plt.xlim(np.min(t),np.max(t))
plt.xlabel("Waktu Pengamatan (JD)")
plt.ylabel("$\Delta$B")
...
for w in ws:
	...
	a2 = np.sqrt(1./(sum_sin2x - (a0**2)*(sum_sinx**2) - (a1**2)*M**2))
	c1 = a1*sum_f_cos
...
ax2 = ax1.twinx()
ax2.plot(np.arange(s,wa,s),C,"k-.", lw=1.2, label="Confidence Level (%)")
fig.tight_layout()
...
ax1.set_title("Kandidat Periode untuk Bintang SV Vul dengan Confidence Level")
# ax1.set_title("Kandidat Periode untuk Bintang SV Vul")
...
plt.xlabel('fase') 
plt.ylabel('$\Delta$B')
plt.title('Kurva Cahaya SV Vul')

## Printing hasil perhitungan
# Print Periode
print ("Frequency Nyquist = {:.3f}".format(wa))
print ("Freuency dari bintang SV Vul adalah {:.3f}".format(ws[np.argmax(H)]))
print ("Periode dari bintang SV Vul adalah {:.3f}".format(P))
# Print data
print ("\nTabel 1. Data SV Vul")
print ("============================================")
